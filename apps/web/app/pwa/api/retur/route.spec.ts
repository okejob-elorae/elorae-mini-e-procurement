...
function formRequest(opts: { omitFile?: boolean; fileType?: string } = {}): Request {
  ...
  fd.set(
    "lines",
    JSON.stringify([{ itemId: "item-1", variantSku: "SKU-1", qty: 1, reason: "DAMAGED" }])
  );
  if (!opts.omitFile) {
...
beforeEach(() => {
  ...
  mockCreate.mockResolvedValue({ returnId: "r1", docNo: "RET/2026/08/0001" });
});
...
describe("POST /pwa/api/retur", () => {
  ...
  it.each<FieldReturnErrorCode>(["NO_LINES", "BAD_QTY", "VISIT_NOT_OWNED", "MISSING_REASON_NOTE"])(
    "maps %s onto 400",
    async (code) => {
  ...
  it("uploads then creates, and returns the docNo", async () => {
    ...
    expect(await res.json()).toMatchObject({ docNo: "RET/2026/08/0001" });
