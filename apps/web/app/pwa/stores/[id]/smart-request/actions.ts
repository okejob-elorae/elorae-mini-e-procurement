...
export async function buildSmartRequestAction(input: unknown): Promise<BuildSmartRequestResult> {
  ...
  const store = await prisma.store.findUnique({ where: { id: parsed.data.storeId }, select: { termsType: true, markupPercent: true, priceDiscountPercent: true } });
  ...
  const lines: SmartRequestLine[] = plan.lines.map((l) => {
    ...
    const unitPrice = computeStorePrice({ sellingPrice, termsType: "PUTUS", markupPercent: markup, priceDiscountPercent: priceDiscount }).price ?? 0;
    return {
      itemId: l.itemId,
