...
function applyPrimary(hex){var s=document.documentElement.style;var brandFg=lum(hex)>0.5?'#111827':'#f9fafb';s.setProperty('--sidebar',hex);s.setProperty('--sidebar-foreground',brandFg);s.setProperty('--sidebar-primary',hex);s.setProperty('--sidebar-primary-foreground',brandFg);var interactive=hex;if(dark&&lum(hex)<0.55){var cur=hex;for(var i=0;i<10;i++){var n=cur.replace('#','');var r=parseInt(n.slice(0,2),16),g=parseInt(n.slice(2,4),16),b=parseInt(n.slice(4,6),16);function mix(c){return Math.round(c+(255-c)*0.18);}function hx(v){return v.toString(16).padStart(2,'0');}cur='#'+hx(mix(r))+hx(mix(g))+hx(mix(b));if(lum(cur)>=0.55)break;}interactive=cur;}var iFg=lum(interactive)>0.5?'#111827':'#f9fafb';s.setProperty('--primary',interactive);s.setProperty('--ring',interactive);s.setProperty('--primary-foreground',iFg);}
