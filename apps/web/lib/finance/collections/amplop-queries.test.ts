...
d("amplop queries (test bed only)", () => {
  ...
  beforeEach(async () => {
    ...
    collectorUserId = ""; salesmanUserId = ""; adminUserId = ""; bothRoleUserId = ""; multiStoreUserId = "";
    collectorStoreId = ""; salesmanStoreId = ""; bothStoreId = ""; storeHighId = ""; storeMedId = ""; storeLowId = "";
    ...
    const [collectorUser, salesmanUser, adminUser, bothRoleUser, multiStoreUser] = await Promise.all([
    ...
    const [collectorStore, salesmanStore, bothStore, storeHigh, storeMed, storeLow] = await Promise.all([
    ...
    /* Three stores, all collected by multiStoreUserId, with three distinct totalOverdue figures. */
  ...
  afterEach(async () => {
    ...
    const storeIds = [collectorStoreId, salesmanStoreId, bothStoreId, storeHighId, storeMedId, storeLowId].map(seededId);
    await prisma.store.deleteMany({ where: { id: { in: storeIds } } });

    const userIds = [collectorUserId, salesmanUserId, adminUserId, bothRoleUserId, multiStoreUserId].map(seededId);
  ...
  it("orders stores by total overdue descending", async () => {
    ...
    /* Pins the three seeded figures are genuinely distinct, so the sort actually had work to do. */
    expect(overdues).toEqual([1000, 300, 0]);
  });
