...
- **Field sales** — Field-sales orders on PUTUS (sold outright, stock leaves at delivery, raises a `Receivable`) or KONSI (consignment, stock reserved at approve and transferred to a per-store `StoreStock` ledger when each delivery shipment completes, no receivable) terms; canvassing van sales and van reconciliation; SPG in-store sales; store stocktakes; konsi sell-through reports (what a KONSI store sold per counting period, from its closing full count, with POS-vs-count gaps held for resolution — no invoicing yet); per-store assortment and konsi suggestions. All of it is per-variant. The PWA (`/pwa/*`) is the field surface — orders, van, SPG, stores, collections, notifications — and works offline.
