...
An ERP for textile and garment manufacturing, covering three domains: **procurement and production** (suppliers, purchase orders, goods receipt, inventory, work orders, vendor returns, supplier payments), **field sales** (field-sales orders putus/konsi, canvassing van sales, SPG store sales, store stocktakes, konsi sell-through reports, field returns — with a PWA for the people in the field), and **finance** (AR ledger, payment recording, collections, store settlement, general ledger and faktur pajak). Plus settings: UOM, tax, document numbering, RBAC.
...
- **Field sales** — Field-sales orders on PUTUS (sold outright, stock leaves at delivery, raises a `Receivable`) or KONSI (consignment, stock reserved at approve and transferred to a per-store `StoreStock` ledger when each delivery shipment completes, no receivable until the store's sell-through is invoiced) terms; canvassing van sales and van reconciliation; SPG in-store sales; store stocktakes; konsi sell-through reports (what a KONSI store sold per counting period, from its closing full count, with POS-vs-count gaps held for resolution; approving a report invoices the period — receivable, faktur, revenue/COGS/shrinkage journals and a printable nota tagihan — or, for a store's first report only, records it as a no-invoice baseline; the store's latest approved report can be voided — receivable voided, faktur cancelled, journals reversed on their original dates — and a corrected one created from the same count); a monthly count schedule for active KONSI stores with a sell-through method (a daily sweep opens a DRAFT count for the month, at most once per store and month, and alerts when it is overdue; the count is full only if every line is counted, approving a full count creates the store's report automatically, and an approved partial count creates none); per-store assortment and konsi suggestions. All of it is per-variant. The PWA (`/pwa/*`) is the field surface — orders, van, SPG, stores, collections, notifications — and works offline.
...
- **AR ledger & payment recording** — Piutang ledger (`/backoffice/finance/piutang`) with one `Receivable` per field-sales (putus) nota tagihan or konsi sell-through report invoiced above zero, and a six-bucket aging view (current through 120+ days); payment recording (`/backoffice/finance/payments`) with multi-receivable allocation, proof upload, and void with a reversing GL entry — the ledger carries five `PaymentMethod` values (cash, transfer, retur-offset, program-deduction, admin-fee), of which manual recording deliberately offers only cash and transfer, the other three being posted by the retur-offset sheet and by store settlement approval; four auto-journals (delivery revenue/COGS, payment receipt/void) posted best-effort with a manual retry, plus the konsi sell-through revenue/COGS/shrinkage journals and their void reversals, retried from the report's own page. Credit limit per store with an approve-time live check and a reason-logged override. Daily overdue sweep raising an admin notification per receivable at its highest newly-crossed threshold. Retur-offset payment settles a receivable with an approved retur's frozen value, released back if the payment is voided. Does **not** cover: giro/cek payment or its clearing state, write-off (the `WRITTEN_OFF` status exists in the schema and every writer respects it as terminal, but no writer *sets* it — it is reachable only by hand SQL), unapplied credit / on-account balances, a credit note or date correction for a konsi sell-through invoice (a wrong one is voided whole and recreated; a konsi transfer is still a stock move, not a sale — konsi AR starts only at sell-through invoicing), or historical GL backfill (pre-existing deliveries got a `Receivable` row but no revenue/COGS journal).
...
- **Store settlement (pelunasan)** — A salesman files a settlement at the counter from the PWA (`/pwa/pelunasan/[storeId]`, reached from the Amplop Digital store card): the invoices being settled — putus notas tagihan, or at a konsi store its invoiced sell-through reports, which settle the same way — the deductions the store negotiated (retur credit, trade-program allowance, a percentage admin fee charged on the netted base), and the cash actually handed over. Finance works the queue at `/backoffice/finance/pelunasan`, where a per-settlement checklist re-runs every writer guard read-only before Approve is enabled; approval posts one payment per non-zero component against the settlement's own invoices oldest-due-first, decrements AR through `recordPayment`, and posts a receipt journal per payment, while reject closes the document with a reason and pushes a notification to the salesman. Filing runs on `settlements:submit`, approving on `collections:manage`. The paper receipt is the **BKM** (Bukti Kas Masuk, whose number IS the settlement's `docNo`): an HTML builder printed from two entry points — the salesman's PWA view at `/pwa/bkm/[settlementId]`, reached from the filing success screen, and a Print BKM button on the finance approval detail. Both are load-bearing, because `pwaAccessGuard` redirects any wildcard holder off `/pwa` and an ADMIN therefore can never open the PWA one. Does **not** cover: a PWA list or history surface for settlements — the BKM is reachable only from the success screen that just filed it or by knowing its URL, so a closed tab loses the id and a salesman cannot navigate back to a BKM he filed yesterday; editing or resubmitting a rejected settlement; or a store handing over more than expected — an over-tender is refused at approval and no override can clear it, because there is no unapplied-credit concept in the AR ledger.
- **General ledger & faktur pajak** — Chart of accounts, account mapping, journals, cash-flow sections and finance reports; marketplace settlements. Faktur pajak tracking (`/backoffice/finance/faktur-pajak`) with NPWP/PPN snapshotted at creation (never a live lookup), a "Sent to Store" state, and a terminal "Cancelled" state set when its konsi sell-through report is voided.
