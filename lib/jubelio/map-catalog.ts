...
/**
 * Unique Item.sku for a Jubelio product line. Uses the 6-char family when it is
 * unique in the payload; otherwise appends `-G{item_group_id}` (e.g. 240000-G109).
 */
...
/**
 * Flatten Jubelio groups into ERP catalog drafts — one Item per Jubelio item_group.
 * Color-only style segments (e.g. 24000040T–24000047T) merge under family parent SKU.
 */
export function buildCatalogDrafts(
  ...
  const filterIds = opts?.itemGroupIds?.length
    ? new Set(opts.itemGroupIds)
    : null;
  ...
  for (const group of payload.data) {
    ...
    for (const variant of group.variants ?? []) {
      if (variant.is_bundle) {
        warnings.push(
          `Skipped bundle variant ${variant.item_code} (group ${group.item_group_id})`
        );
        continue;
      }
      ...
      if (!acc.familyCode) {
        ...
        warnings.push(
          `Group ${group.item_group_id}: mixed families ${acc.familyCode} and ${family} in ${code}`
        );
      }
  ...
  for (const acc of byGroup.values()) {
    if (acc.familyCode) {
      familyGroupCounts.set(
        acc.familyCode,
        (familyGroupCounts.get(acc.familyCode) ?? 0) + 1
      );
    }
  }
  ...
  for (const acc of byGroup.values()) {
    ...
    const variantless =
      acc.variants.length === 1 && isVariantlessItemCode(acc.variants[0].item_code);
    ...
    const categoryId =
      jubelioCategoryId != null
        ? opts?.categoryIdByJubelioId?.get(jubelioCategoryId) ?? null
        : null;
...
export function loadSnapshotPayload(snapshotPath: string): JubelioItemsPayload {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const fs = require('fs') as typeof import('fs');
  const raw = fs.readFileSync(snapshotPath, 'utf-8');
  return JSON.parse(raw) as JubelioItemsPayload;
}
