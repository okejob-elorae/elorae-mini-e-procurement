  ...
  useEffect(() => {
    ...
    getItemById(finishedGoodId)
      .then((item: { variants?: unknown } | null) => {
        ...
        // Apply WO variant selection after options are loaded so the combobox can display it
        if (wo && wo.status === 'DRAFT') {
          const sku = (wo as { skuBreakdown?: { variantSku?: string; attributes?: Record<string, string> } }).skuBreakdown;
          if (sku && typeof sku === 'object' && sku.variantSku) {
            setSelectedVariantSku(sku.variantSku);
            setSelectedVariantAttributes(sku.attributes ?? null);
          }
        }
      })
