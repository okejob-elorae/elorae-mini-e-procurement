...
- Adding a `DocType` enum member? Two `Record<DocType, …>` maps must both get it — `apps/web/lib/docNumber.ts` and `apps/web/app/actions/settings/doc-numbers.ts` — missing either only breaks in the Docker build during deploy, after migrations have already applied.
