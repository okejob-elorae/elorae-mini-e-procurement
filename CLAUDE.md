...
- Correcting a delivery's dates? `updateDeliveryDatesAction` moves THREE rows (delivery, `Receivable`, both posted journals) in one transaction, and REFUSES the correction outright once the receivable has `paidAmount > 0` — void the payment first. See `docs/ARCHITECTURE-NOTES.md`.
