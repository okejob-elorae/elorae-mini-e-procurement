...
| **7 (Photos)** | Photo documentation on visits (17-07) — `VisitPhoto` table (`clientId @unique`, `visitId` idx) + `attachVisitPhoto` writer (`clientId`-idempotent, visit-ownership via `findFirst` on id+userId, no open-check → attaches post-checkout) + auth'd `POST /pwa/api/upload/visit-photo` (401→403→503→400; `clientId` + ownership pre-checks BEFORE `uploadToR2`; key `visit-photos/<visitId>/<clientId>.jpg`; MIME→ext) + backoffice read-only gallery (thumbnail strip + lightbox on store detail). PWA: client-side compression (`@/lib/pwa/photo/compress` canvas 1600px/0.8 → ~200-400 KB), offline `pendingPhotos` Dexie queue (v2) + `flushPendingPhotos` reusing `setupOrderSync` (orders→photos, `clientId=localId` server-dedup), `VisitPhotoCapture` (getUserMedia live viewfinder — **stream attached in a post-mount effect, NOT synchronously, or the feed renders black** — + `<input capture>` fallback, soft cap 20, per-visit strip synced∪pending with retry, blob-URL revoke, `router.refresh()` on successful sync). Reuses R2 (`@/lib/r2`). Check-in now lands on the store detail (the visit workspace). | ✅ shipped (PR #118 backend+gallery, #119 PWA capture+offline — merged 2026-07-06; smoke fixes #120 check-in redirect→store-detail, #121 camera stream-attach — merged 2026-07-07). Verified on prod. Follow-ups (non-blocking): soft-cap race (advisory; server has no cap); 401-mid-sync marks pending `failed` (recoverable via retry — consider treating auth as transient); lightbox `DialogTitle` + capture strings hardcoded ID (i18n debt). |
