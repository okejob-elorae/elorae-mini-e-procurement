...
- Reading/writing `VanStock`, `StoreStock` or `KonsiTransferLine`? A THIRD variant-key spelling exists — `VanStock.variantSku` is nullable too (writers insert `""`, but `null` is legal), while `StoreStock`/`KonsiTransferLine` are non-nullable `@default("")`. Stay OR-tolerant on the nullable ones, normalise `?? ""` on write into the non-nullable ones. `StoreStock` has FOUR writers now — the konsi transfer (in), a konsi retur approve (out), a KONSI-store SPG sale (out), and a store stocktake approval, which SETS the quantity rather than adjusting it. Only the transfer blends `avgCost`; every other writer, the stocktake included, leaves it alone — a decrement can go negative on purpose and a stocktake-created line lands at `avgCost: 0`, same as an SPG-sale-created negative row. A drifted or negative row is now correctable, but only at a store where someone actually opens and approves a count — nothing schedules one. **The konsi-retur decrement (out) is itself origin-dependent as of PR #258**: a FIELD-origin return still decrements the full `creditedQty` once, at approve; an ADMIN-origin return decrements `receivedQty` at RECEIPT and only the delta (`creditedQty − receivedQty`) at approve — reading either writer alone looks like the decrement was halved or doubled, but the total over the return's life is `creditedQty` either way for both origins. ADMIN decrements early so the store's ledger keeps the units until they physically arrive rather than vanishing into an in-transit void — the cost is that the ledger OVERSTATES during that transit window, so a stocktake opened in it reports a shortfall that is really goods on a truck; the store detail card shows the in-transit qty, the stocktake count screen deliberately does not, since `expectedQty` must stay the live ledger figure and never gain a second meaning. See `docs/ARCHITECTURE-NOTES.md`.
